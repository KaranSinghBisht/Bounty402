...
contract Bounty402Test is Test {
    ...
    function testClaimWithAttestation() public {
        uint256 validatorPk = 0xBEEF;
        ...
        vm.prank(alice);
        uint256 id = b.createBountyWithValidator(IERC20(address(token)), 100e6, deadline, specHash, validator);

        vm.prank(bob);
        uint256 sid = b.submitWork(id, keccak256("artifact"), "ipfs://fake");

        bytes32 digest = b.attestationDigest(id, sid, bob, keccak256("artifact"));
        ...
        uint256 bobBefore = token.balanceOf(bob);
        vm.prank(bob);
        b.claimWithAttestation(id, sid, sig);
